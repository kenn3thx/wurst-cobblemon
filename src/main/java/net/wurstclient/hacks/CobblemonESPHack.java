...
public final class CobblemonESPHack extends Hack implements UpdateListener,
	...
	private final CheckboxSetting filterShiny = new CheckboxSetting("Show Shiny", "Highlight shiny Pokemon.", true);

	private final CheckboxSetting filterLegendary = new CheckboxSetting(
			"Show Legendary", "Highlight legendary and mythical Pokemon.", true);

	private final CheckboxSetting filterUltraRare = new CheckboxSetting(
			"Show Ultra-Rare", "Highlight ultra-rare Pokemon.", true);

	private final CheckboxSetting filterRare = new CheckboxSetting("Show Rare", "Highlight rare Pokemon.", true);

	private final CheckboxSetting filterUncommon = new CheckboxSetting(
			"Show Uncommon", "Highlight uncommon Pokemon.", true);

	private final CheckboxSetting filterCommon = new CheckboxSetting("Show Common", "Highlight common Pokemon.", false);

	private final TextFieldSetting whitelist = new TextFieldSetting("Whitelist",
			"Comma-separated list of Pokemon names to highlight.\n"
					+ "Leave empty to show all based on rarity filters.",
			"");
	...
	public void onUpdate() {
		...
		stream = stream.filter(e -> {
			...
			if ((pokemon.isLegendary() || pokemon.isMythical())
					&& filterLegendary.isChecked())
				return true;

			if (pokemon.isUltraBeast() && filterUltraRare.isChecked())
				return true;

			// Add more specific rarity logic here as needed based on Cobblemon
			// API
	...
	public void onRender(PoseStack matrixStack, float partialTicks) {
		...
		if (style.hasLines()) {
			...
			for (PokemonEntity e : pokemonList) {
				Vec3 point = EntityUtils.getLerpedBox(e, partialTicks).getCenter();
